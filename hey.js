...
const STACKS_NODE_URL =
  process.env.STACKS_NODE_URL || "https://stacks-node-api.mainnet.stacks.co";
  ...
  const interval = setInterval(async () => {
    try {
      const txData = await fetchWithRetry(
        `${STACKS_NODE_URL}/extended/v1/tx/${txId}`
      );
      if (txData.tx_status === "success" && !confirmedTransactions.has(txId)) {
        ...
        const message = `✅ *Transaction Confirmed!* ✅\n\n🔗 Contract ID: ${
          txData.smart_contract?.contract_id || "N/A"
...
async function checkMempool() {
  ...
  try {
    const response = await fetchWithRetry(
      `${STACKS_NODE_URL}/extended/v1/tx/mempool?limit=20&unanchored=true&order_by=age&order=desc`
    );
    ...
    if (smartContracts.length > 0) {
      console.log(
        `Found ${smartContracts.length} relevant smart contract transaction(s).`
      );
